...
ALTER TABLE available_instrument ADD CONSTRAINT PK_available_instrument PRIMARY KEY (available_instrument_id,price);
...
BEGIN
    ...
    IF EXISTS (SELECT 1
        FROM rental r
        WHERE r.student_id = NEW.student_id AND r.available_instrument_id = NEW.available_instrument_id AND r.status = 'Active')
        THEN RAISE EXCEPTION 'Student already has that rental';
    END IF;

    IF (SELECT CAST(ai.quantity AS INT) FROM available_instrument ai WHERE ai.available_instrument_id = NEW.available_instrument_id) = 0 THEN
        RAISE EXCEPTION 'Quantity for that instrument is 0';
    END IF;
...
CREATE OR REPLACE FUNCTION calculate_lesson_type_skill_id_and_get_price()
RETURNS TRIGGER AS $$
BEGIN
    ...
    UPDATE attended_lessons
    SET price = (
        SELECT lp.price
        FROM lesson_pricing lp
        WHERE lp.pricing_lesson_type_skill_id = (
            SELECT l.lesson_type_skill_id
            FROM lesson l
            WHERE l.lesson_id = NEW.lesson_id
        )
        AND lp.valid_from <= NEW.date
        ORDER BY lp.valid_from DESC
        LIMIT 1
    )
    WHERE attended_lessons.attended_lessons_id = NEW.attended_lessons_id;
...
CREATE TRIGGER trg_calculate_lesson_type_skill_id_and_get_price
AFTER INSERT ON attended_lessons
FOR EACH ROW
EXECUTE FUNCTION calculate_lesson_type_skill_id_and_get_price();
...
CREATE OR REPLACE FUNCTION update_attended_lessons_individual()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.status = 'ended' THEN
        INSERT INTO attended_lessons (student_id, instructor_id, lesson_id, date)
        VALUES (NEW.student_id, NEW.instructor_id, NEW.lesson_id, NEW.start_time);
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trg_update_attended_lessons_individual
AFTER INSERT OR UPDATE OF status ON individual_lesson
FOR EACH ROW
WHEN (NEW.status = 'ended')
EXECUTE FUNCTION update_attended_lessons_individual();
...
CREATE OR REPLACE FUNCTION update_attended_lessons()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.status = 'ended' THEN
        INSERT INTO attended_lessons (student_id, instructor_id, lesson_id, date)
...
CREATE TRIGGER trg_update_attended_lessons
AFTER INSERT OR UPDATE OF status ON schedule_booking
FOR EACH ROW
WHEN (NEW.status = 'ended')
EXECUTE FUNCTION update_attended_lessons();
...
ALTER TABLE rental ADD CONSTRAINT FK_rental_1 FOREIGN KEY (available_instrument_id, price) REFERENCES available_instrument (available_instrument_id, price);
